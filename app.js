...
function renderLiqCard(liq, price, dec) {
  if (!liq) return '';
  const fn2 = (n) => n ? parseFloat(n).toLocaleString('en-US', {minimumFractionDigits: dec, maximumFractionDigits: dec}) : '—';
  ...
  return `<div class="card full" style="border-color:rgba(167,139,250,0.25)">
    ...
    <div style="padding:8px 12px;background:rgba(167,139,250,.05);border:1px solid rgba(167,139,250,.2);border-radius:6px;font-size:11px;color:var(--text2);font-family:var(--mono);line-height:1.6;margin-bottom:12px">
    ...
    <div style="font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:var(--purple);font-family:var(--mono);margin-bottom:8px">⚡ Liquidity sweep entry setups</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">

      <!-- Long sweep entry -->
      <div style="background:rgba(0,208,132,0.05);border:1px solid var(--green-b);border-radius:8px;padding:12px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--green);font-family:var(--mono);margin-bottom:8px">🟢 Long after sweep</div>
        <div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-bottom:6px">${liq.longSweepEntry.logic}</div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Enter at</span>
          <span style="font-size:13px;font-weight:600;color:var(--blue);font-family:var(--mono)">$${fn2(liq.longSweepEntry.entry)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Stop</span>
          <span style="font-size:12px;color:var(--red);font-family:var(--mono)">$${fn2(liq.longSweepEntry.stop)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP1</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.longSweepEntry.tp1)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP2</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.longSweepEntry.tp2)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP3 (major)</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.longSweepEntry.tp3)}</span>
        </div>
        <div style="margin-top:8px;padding:5px 8px;background:rgba(0,208,132,.1);border-radius:4px;font-size:11px;font-family:var(--mono);color:var(--green)">
          Wait for sweep candle, then enter on next 1H close
        </div>
      </div>

      <!-- Short sweep entry -->
      <div style="background:rgba(255,77,77,0.05);border:1px solid var(--red-b);border-radius:8px;padding:12px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--red);font-family:var(--mono);margin-bottom:8px">🔴 Short after squeeze</div>
        <div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-bottom:6px">${liq.shortSweepEntry.logic}</div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Enter at</span>
          <span style="font-size:13px;font-weight:600;color:var(--blue);font-family:var(--mono)">$${fn2(liq.shortSweepEntry.entry)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Stop</span>
          <span style="font-size:12px;color:var(--red);font-family:var(--mono)">$${fn2(liq.shortSweepEntry.stop)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP1</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.shortSweepEntry.tp1)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP2</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.shortSweepEntry.tp2)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:4px 0">
          <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP3 (major)</span>
          <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(liq.shortSweepEntry.tp3)}</span>
        </div>
        <div style="margin-top:8px;padding:5px 8px;background:rgba(255,77,77,.1);border-radius:4px;font-size:11px;font-family:var(--mono);color:var(--red)">
          Wait for squeeze candle, then enter on next 1H close
        </div>
      </div>
    </div>
  </div>`;
...
async function fetchAI(coin,ta,sc,setup,klines,mtfData,liqZones){
...
async function renderDetail(coin,klines,mtfData){
  ...
  const liqZones=calcLiqZones(d.price,ta.atr,d.oi,d.funding,d.oiNotional);
  ...
  renderScanBody();
  loadAI(coin,ta,sc,setup,klines,mtfData,liqZones);
}
...
async function init(){
  ...
  try{
    // Fetch active coin first, then others in background
    const klines=await fetchKlines(activeCoin,activeTF);
    // Fetch other coins in background for accurate RSI/trend in ranking
    Promise.allSettled(
      Object.keys(COINS).filter(c=>c!==activeCoin).map(c=>fetchKlines(c,activeTF))
    ).then(()=>{
      const bg={};
      Object.keys(COINS).forEach(c=>{bg[c]=scoreSignal(c,calcTA(klCache[c+'_'+activeTF]||[]),null);});
      renderSidebar(bg);renderAlerts(bg);
    });
    let mtfData=null;
    try{
...
async function loadAI(coin,ta,sc,setup,klines,mtfData,liqZones){
  const ai=await fetchAI(coin,ta,sc,setup,klines,mtfData,liqZones);
