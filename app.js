  ...
  try {
    ...
    function buildCard(side, zones) {
      ...
      const tp1 = isLong
        ...
        : (ta.support    || +(price * 0.982).toFixed(dec));
      const tp2 = isLong
        ? +(price * 1.038).toFixed(dec)
        : +(price * 0.962).toFixed(dec);

      // Proximity: only show if price is within 1.5 ATR of nearest cluster level
      const nearest = isLong ? clusterTop : clusterTop;
      ...
      return '<div class="card" style="border-color:'+borderCol+';background:rgba(0,0,0,0.02);flex:1;min-width:300px">'
        ...
        + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono)">below cluster</div></div>'
...
function detectPostSweepConfirmation(klines, liqZones, dec) {
  ...
  const prev = klines[klines.length - 2];  // the sweep candle (completed)
  const curr = klines[klines.length - 1];  // current candle (confirmation close)
  if (!prev || !curr) return null;

  // Long sweep: prev wick went below long liq zone AND closed back above
  const longZones = liqZones.longLiqZones || [];
  for (const zone of longZones) {
    if (prev.l <= zone.price && prev.c > zone.price) {
      // Confirmed long sweep — curr should be green (closing higher)
      ...
      return {
        ...
        sweepCandle: prev,
        sweepLow:   prev.l,
        sweepLevel: zone.price,
        leverage:   zone.leverage,
        entryPrice: +curr.c.toFixed(dec),
        stopPrice:  +(prev.l * 0.998).toFixed(dec),   // just below sweep wick low
        stopPct:    +((curr.c - prev.l * 0.998) / curr.c * 100).toFixed(3),
        tp1:        +(curr.c * 1.0045).toFixed(dec),  // 0.45% — 45% ROI at 100x
        tp2:        +(curr.c * 1.009).toFixed(dec),   // 0.9%  — 90% ROI at 100x
        tp3:        +(curr.c * 1.018).toFixed(dec),   // 1.8%  — back to resistance
        sweepDepth, recovery,
  ...
  // Short squeeze: prev wick went above short liq zone AND closed back below
  ...
  for (const zone of shortZones) {
    if (prev.h >= zone.price && prev.c < zone.price) {
      ...
      return {
        ...
        leverage:   zone.leverage,
        entryPrice: +curr.c.toFixed(dec),
        stopPrice:  +(prev.h * 1.002).toFixed(dec),
        stopPct:    +((prev.h * 1.002 - curr.c) / curr.c * 100).toFixed(3),
...
function renderPostSweepCard(sweep, dec) {
  ...
  const levNote = activeLev >= 75
    ? '<div style="background:rgba(0,208,132,0.1);border:1px solid var(--green-b);border-radius:6px;padding:8px 12px;margin-top:10px;font-size:11px;color:var(--green);font-family:var(--mono)">Designed for 75x-125x. Stop is '
      + sweep.stopPct.toFixed(3) + '% — within safe range for high leverage.</div>'
  ...
  return '<div class="card full" style="border-color:'+border+';background:'+bg+';border-width:2px">'
    ...
    + '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">'
    + '<span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Stop (below wick)</span>'
...
async function sendSweepAlert(coin, sweep, dec) {
  try {
    const fmt = n => safeFormat(n, dec);
    const isLong = sweep.side === 'long';
    const emoji  = isLong ? 'LONG SWEEP' : 'SHORT SQUEEZE';
    const d = mktData[coin] || {};
    const payload = {
      coin, tf: activeTF,
      price: d.price || sweep.entryPrice,
      score: 8,  // sweep confirmations are always high-value
      bias: isLong ? 'long' : 'short',
      conviction: 'high',
      pattern: emoji + ' CONFIRMED',
      patternStage: 'confirmed',
      patternConfidence: 'high',
      winRate: '',
      longEntry:  isLong ? sweep.entryPrice : sweep.tp3,
      longStop:   isLong ? sweep.stopPrice  : sweep.stopPrice,
      longTP1:    isLong ? sweep.tp1 : sweep.tp2,
      longTP2:    isLong ? sweep.tp2 : sweep.tp1,
      shortEntry: isLong ? sweep.tp3 : sweep.entryPrice,
      shortStop:  isLong ? sweep.stopPrice : sweep.stopPrice,
      shortTP1:   isLong ? sweep.tp2 : sweep.tp1,
      shortTP2:   isLong ? sweep.tp1 : sweep.tp2,
      rsi: '50', trend: 'sweep', funding: (d.funding||0).toFixed(4),
      fgValue: d.fgValue || 50, leverage: activeLev,
      liqLong: sweep.sweepLevel, liqShort: sweep.sweepLevel,
      stopDistPct: sweep.stopPct, maxSafeStopPct: 0.5,
      longMaxSafeLev: 125, shortMaxSafeLev: 125,
      suggestedAction: 'Enter at $'+fmt(sweep.entryPrice)+' stop $'+fmt(sweep.stopPrice)+' TP1 $'+fmt(sweep.tp1)+' TP2 $'+fmt(sweep.tp2),
      watchLevel: fmt(sweep.sweepLevel)
    };
...
async function sendTelegramAlert(coin, ta, sc, setup, ai, d, dec) {
  try {
    ...
    const patternReady = patternStage === 'confirmed' || patternStage === 'near breakout' || patternStage === 'forming';
    const confOk       = patternConf === 'high' || (patternConf === 'medium' && sc >= 7);
    if (sc < 7 || !confOk || !patternReady) {
      console.log(`Alert skipped: score ${sc}/10, conf ${patternConf}, stage ${patternStage}`);
      return;
    }
