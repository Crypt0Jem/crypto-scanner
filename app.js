  ...
  try {
    ...
    return `<div class="card full" style="border-color:rgba(167,139,250,0.3);background:rgba(167,139,250,0.03)">
      <div class="card-title" style="color:var(--purple)">\u26A1 Liquidity sweep entry setups
        <span style="margin-left:8px;font-size:10px;color:var(--text3);font-family:var(--mono)">Enter AFTER the sweep confirms, not before</span>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div style="background:rgba(0,208,132,0.05);border:1px solid var(--green-b);border-radius:8px;padding:14px">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            ...
            <span style="font-size:12px;color:var(--red);font-family:var(--mono)">$${fn2(ls.stop)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP1</span>
            <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(ls.tp1)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP2</span>
            <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(ls.tp2)}</span>
          </div>
        ...
        <div style="background:rgba(255,77,77,0.05);border:1px solid var(--red-b);border-radius:8px;padding:14px">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Stop</span>
            <span style="font-size:12px;color:var(--red);font-family:var(--mono)">$${fn2(ss.stop)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP1</span>
            <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(ss.tp1)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP2</span>
            <span style="font-size:12px;color:var(--green);font-family:var(--mono)">$${fn2(ss.tp2)}</span>
          </div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0">
            <span style="font-size:11px;color:var(--text2);font-family:var(--mono)">TP3 (major)</span>
...
function calcCVD(klines) {
  const empty = { deltas:[], cvd:[], divergence:null, trend:'neutral', cvdValue:0, cvdChange:0 };
  if (!klines || klines.length < 5) return empty;
  const deltas = klines.map(k => {
    const range = k.h - k.l;
    if (range === 0) return 0;
    const buyPct = (k.c - k.l) / range;
    return k.v * (2 * buyPct - 1);
  });
  let cum = 0;
  ...
  const cvdOld    = cvd[cvd.length - 1 - lookback];
  const cvdCurr   = cvd[cvd.length - 1];
  const priceChgPct = priceOld !== 0 ? (priceCurr - priceOld) / priceOld : 0;
  const cvdChange   = cvdCurr - cvdOld;
  ...
  if (Math.abs(priceChgPct) > 0.003) {
    if (priceChgPct > 0 && cvdChgPct < -0.12)
      divergence = { type:'bearish', label:'\u26A0 Bearish CVD div', desc:'Price rising but cumulative sell pressure increasing \u2014 hidden distribution, potential reversal' };
    else if (priceChgPct < 0 && cvdChgPct > 0.12)
      divergence = { type:'bullish', label:'\uD83D\uDFE2 Bullish CVD div', desc:'Price falling but cumulative buy pressure increasing \u2014 hidden accumulation, potential reversal' };
  }
  const trend = cvdChange > 0 ? 'bullish' : cvdChange < 0 ? 'bearish' : 'neutral';
  return { deltas, cvd, divergence, trend, cvdValue:cvdCurr, cvdChange, priceChgPct };
}
...
function buildCVDChart(klines, cvdData) {
  ...
  if (!canvas || !cvdData || cvdData.cvd.length < 5) return;
  const ctx = canvas.getContext('2d');
  const dpr = window.devicePixelRatio || 1;
  const wrap = canvas.parentElement;
  canvas.width  = wrap.clientWidth  * dpr;
  ...
  ctx.scale(dpr, dpr);
  const W = wrap.clientWidth, H = wrap.clientHeight;
  const PAD = { t:22, r:68, b:16, l:8 };
  const cW = W - PAD.l - PAD.r;
  const totalH = H - PAD.t - PAD.b;
  const cvdH = totalH * 0.54, gapH = totalH * 0.06, barH = totalH * 0.40;
  ...
  ctx.textAlign = 'left';
  ctx.fillText('CVD LINE', PAD.l, cvdTop - 6);
  ctx.fillText('DELTA BARS', PAD.l, barTop - 4);
  ...
  deltas.forEach((d, i) => {
    ...
    ctx.fillStyle = d >= 0 ? 'rgba(0,208,132,0.75)' : 'rgba(255,77,77,0.75)';
  ...
  grad.addColorStop(0, isUp ? 'rgba(0,208,132,0.18)' : 'rgba(255,77,77,0.18)');
  ...
  // Right-side labels
  const fmt = n => { const a=Math.abs(n); return a>=1e9?(n/1e9).toFixed(1)+'B':a>=1e6?(n/1e6).toFixed(1)+'M':a>=1e3?(n/1e3).toFixed(1)+'K':n.toFixed(0); };
  ctx.font = '9px DM Mono, monospace'; ctx.textAlign = 'left';
  ctx.fillStyle = lineCol; ctx.fillText(fmt(cvdVals[n-1]), W-PAD.r+4, cvdY(cvdVals[n-1])+3);
  ctx.fillStyle = '#555';
  ...
  if (cvdData.divergence) {
    ...
    ctx.fillText(cvdData.divergence.label, W-PAD.r-6, cvdTop+10);
  }
}
