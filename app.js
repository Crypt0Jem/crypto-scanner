...
function renderLiqScaleInCard(coin, tf, price, ta, klines, cvdData, dec, liqZones) {
  try {
    ...
    // Use already-calculated liqZones levels (stable pivot-anchored)
    // Fall back to current-price calculation if liqZones not available
    let clusterTop, clusterMid, clusterBottom, stopLoss;
    if (liqZones && liqZones.longLiqZones && liqZones.longLiqZones.length >= 3) {
      // Use the pre-calculated zone levels from renderDetail
      const zones = liqZones.longLiqZones;
      ...
      clusterTop    = z125.price;
      clusterMid    = z50.price;
      clusterBottom = z25.price;
      stopLoss      = +(z10.price - atr * 0.5).toFixed(dec);
    } else {
      // Fallback: calculate from current price
      clusterTop    = +(price * (1 - (1/125 - 0.005 + 0.0005))).toFixed(dec);
      clusterMid    = +(price * (1 - (1/50  - 0.004 + 0.0005))).toFixed(dec);
      clusterBottom = +(price * (1 - (1/25  - 0.004 + 0.0005))).toFixed(dec);
      stopLoss      = +(price * (1 - (1/10  - 0.003 + 0.0005)) - atr * 0.5).toFixed(dec);
    }
    const zoneRange = +(clusterTop - clusterBottom).toFixed(dec);

    // Only show when price is within 1.5 ATR of cluster top
    if (Math.abs(price - clusterTop) > atr * 1.5) return '';

    // TP levels: resistance for TP1, short liq cluster for TP2
    const tp1 = ta.resistance || +(price * 1.018).toFixed(dec);
    const tp2 = +(price * 1.038).toFixed(dec);

    // Confidence filters (reuse existing data)
    const d = mktData[coin] || {};
    const volMult = getVolumeMultiple(klines);
    const isVolumeQuiet    = volMult < 1.8;                     // no spike yet
    const isCVDBearish     = cvdData && (cvdData.trend === 'bearish' || cvdData.trend === 'neutral');
    const isFundingPos     = (d.funding || 0) > 0;             // longs heavy = more longs to sweep
    const isAboveEMA       = ta.ema50 ? price > ta.ema50 : price > ta.ema20; // structure intact
    const isNearCluster    = Math.abs(price - clusterTop) < atr; // close enough

    let confScore = 0;
    if (isVolumeQuiet)  confScore++;
    if (isCVDBearish)   confScore++;
    if (isFundingPos)   confScore++;
    if (isAboveEMA)     confScore++;
    ...
    const distPct = ((price - clusterTop) / price * 100).toFixed(2);
    ...
    const alreadySwept = price < clusterTop
      ? '<div style="background:rgba(74,158,255,0.08);border:1px solid var(--blue-b);border-radius:6px;padding:8px 12px;margin-top:10px;font-size:11px;color:var(--blue);font-family:var(--mono)">Already swept? Enter full size at current price, stop below $' + fn2(stopLoss) + '</div>'
      : '';

    const filterRows = [
      { label: 'Volume quiet (no spike)', pass: isVolumeQuiet, val: volMult.toFixed(1) + 'x' },
      { label: 'CVD neutral/bearish', pass: isCVDBearish, val: cvdData ? cvdData.trend : '--' },
      { label: 'Funding positive (longs heavy)', pass: isFundingPos, val: d.funding ? d.funding.toFixed(4)+'%' : '--' },
      { label: 'Price above 50 EMA (structure)', pass: isAboveEMA, val: isAboveEMA ? 'yes' : 'no' },
    ...
    return '<div class="card full" style="border-color:rgba(245,166,35,0.5);background:rgba(245,166,35,0.03)">'
      + '<div class="card-title" style="color:var(--amber);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">'
      + '<span>Liq Zone Scale-In Setup</span>'
      + '<span style="font-size:10px;background:'+confBg+';color:'+confColor+';border:1px solid '+confColor+';border-radius:3px;padding:2px 8px;font-family:var(--mono)">'
      + confScore + '/5  ' + confLabel + ' CONFIDENCE</span>'
      + '</div>'
      + '<p style="font-size:11px;color:var(--text2);font-family:var(--mono);margin:0 0 12px;line-height:1.6">'
      + 'Counter-trend entry inside the liq cluster. Scale in as price sweeps through the zone. '
      + 'Stop below the entire cluster. Use 25-50x max.</p>'

      // Zone range bar
      + '<div style="background:var(--bg3);border-radius:8px;padding:12px;margin-bottom:12px">'
      + '<div style="display:flex;justify-content:space-between;margin-bottom:6px">'
      + '<span style="font-size:10px;color:var(--text3);font-family:var(--mono)">SWEEP ZONE</span>'
      + '<span style="font-size:10px;color:var(--amber);font-family:var(--mono)">$' + fn2(zoneRange) + ' range</span>'
      + '</div>'
      + '<div style="display:flex;justify-content:space-between;font-family:var(--mono)">'
      + '<span style="font-size:13px;color:var(--green)">$' + fn2(clusterTop) + '</span>'
      + '<span style="font-size:10px;color:var(--text3)">-- zone --</span>'
      + '<span style="font-size:13px;color:var(--red)">$' + fn2(clusterBottom) + '</span>'
      + '</div></div>'

      // Scale-in levels
      + '<div style="margin-bottom:12px">'
      + '<div style="font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:var(--text3);font-family:var(--mono);margin-bottom:8px">Scale-in levels</div>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border)">'
      + '<span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Entry 1 — Top of zone (40%)</span>'
      + '<span style="font-size:14px;font-weight:700;color:var(--blue);font-family:var(--mono)">$' + fn2(clusterTop) + '</span></div>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border)">'
      + '<span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Entry 2 — Mid zone (35%)</span>'
      + '<span style="font-size:14px;font-weight:700;color:var(--blue);font-family:var(--mono)">$' + fn2(clusterMid) + '</span></div>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0">'
      + '<span style="font-size:11px;color:var(--text2);font-family:var(--mono)">Entry 3 — Bottom of zone (25%)</span>'
      + '<span style="font-size:14px;font-weight:700;color:var(--blue);font-family:var(--mono)">$' + fn2(clusterBottom) + '</span></div>'
      + '</div>'

      // Stop + TPs
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px">'
      + '<div style="background:rgba(255,77,77,0.07);border:1px solid var(--red-b);border-radius:7px;padding:10px;text-align:center">'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">UNIFIED STOP</div>'
      + '<div style="font-size:14px;font-weight:700;color:var(--red);font-family:var(--mono)">$' + fn2(stopLoss) + '</div>'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-top:2px">below full cluster</div></div>'
      + '<div style="background:rgba(0,208,132,0.07);border:1px solid var(--green-b);border-radius:7px;padding:10px;text-align:center">'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">TP1</div>'
      + '<div style="font-size:14px;font-weight:700;color:var(--green);font-family:var(--mono)">$' + fn2(tp1) + '</div>'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-top:2px">resistance</div></div>'
      + '<div style="background:rgba(0,208,132,0.07);border:1px solid var(--green-b);border-radius:7px;padding:10px;text-align:center">'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">TP2</div>'
      + '<div style="font-size:14px;font-weight:700;color:var(--green);font-family:var(--mono)">$' + fn2(tp2) + '</div>'
      + '<div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-top:2px">short liq zone</div></div>'
      + '</div>'

      // Confidence filter breakdown
      + '<div style="background:var(--bg3);border-radius:7px;padding:10px;margin-bottom:10px">'
      + '<div style="font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:var(--text3);font-family:var(--mono);margin-bottom:6px">Confidence filters</div>'
      + filterRows
      + '</div>'
