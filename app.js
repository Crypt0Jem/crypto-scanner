...
async function init(){
  ...
  seedInitialTrades();
  seedPlaceholders();
  console.log('[INIT] 2 seed done');
  const scores={};Object.keys(COINS).forEach(c=>{scores[c]=5;});
  try{renderSidebar(scores);}catch(e){console.error('[INIT] renderSidebar err',e);}
  renderAlerts(scores);
  ...
  try{
    ...
    const klines = klCache[activeCoin+'_'+activeTF] || [];
    console.log('[INIT] 6 klines len='+klines.length);
    let mtfData=null;
    if(!IS_MOBILE){
