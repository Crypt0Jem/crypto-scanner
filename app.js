...
function buildTVChart(coin,tf){
  ...
  c.innerHTML='';
  // Skip heavy TradingView iframe on mobile — show price link instead
  if(IS_MOBILE){
    var price = (mktData[coin]||{}).price||0;
    var chg = (mktData[coin]||{}).change24h||0;
    var chgCol = chg>=0?'var(--green)':'var(--red)';
    c.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:var(--bg2);border-radius:8px">'
      +'<div><div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">LIVE PRICE · ' + coin + ' · ' + tf.toUpperCase() + '</div>'
      +'<div style="font-size:22px;font-weight:700;font-family:var(--mono)">'+(price?'$'+price.toLocaleString():'Loading...')+'</div>'
      +'<div style="font-size:11px;color:'+chgCol+';font-family:var(--mono)">'+(chg>=0?'+':'')+chg.toFixed(2)+'% 24h</div></div>'
      +'<a href="https://www.tradingview.com/chart/?symbol='+COINS[coin].tv+'" target="_blank" style="font-size:10px;color:var(--blue);font-family:var(--mono);text-decoration:none;padding:6px 12px;border:1px solid var(--blue);border-radius:6px">Open Chart ↗</a>'
      +'</div>';
    return;
  }
