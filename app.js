...
function renderLiqScaleInCard(coin, tf, price, ta, klines, cvdData, dec, liqZones) {
  try {
    ...
    function buildCard(side, zones) {
      ...
      // Long TPs: target short liq cluster above
      ...
      const shortTP2 = +(price * 0.962).toFixed(dec);   // ~3.8% below price (long liq zone)
...
async function renderDetail(coin,klines,mtfData){
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      // Draw canvas after HTML inserted
      if(liqHeatmapOn) drawLiqHeatmapCanvas(coin, d.price, klines);
    },50);
  },100);
  renderScanBody();
...
async function sendSweepAlert(coin, sweep, ta, setup, dec) {
  try {
    ...
    // For alert.js: fill both sides but flag only the active one
    // Long sweep → long setup is real, short setup is placeholder (not a trade)
    // Short squeeze → short setup is real, long setup is placeholder
    const payload = {
      coin, tf: activeTF,
      price:      d.price || entry,
      score:      8,
      ...
      winRate: '',
      // Active direction — real sweep levels
      longEntry:  isLong ? entry : entry,
      longStop:   isLong ? stop  : +(entry * (1 - maxSD)).toFixed(dec),
      longTP1:    isLong ? tp1   : +(entry * 0.9955).toFixed(dec),
      longTP2:    isLong ? tp2   : +(entry * 0.991).toFixed(dec),
      shortEntry: isLong ? +(entry * (1 + maxSD)).toFixed(dec) : entry,
      shortStop:  isLong ? +(entry * (1 + maxSD * 1.6)).toFixed(dec) : stop,
      shortTP1:   isLong ? +(entry * 1.0045).toFixed(dec) : tp1,
      shortTP2:   isLong ? +(entry * 1.009).toFixed(dec)  : tp2,
...
function toggleLiqHeatmap(on) {
  ...
  if (liqHeatmapOn) setTimeout(function() { drawLiqHeatmapCanvas(ctx.coin, price, ctx.klines); }, 60);
}
...
function renderLiqHeatmapCard(coin, currentPrice, klines, dec) {
  ...
  const statusText = heatData.isFallback
    ? 'Estimated (WS queue <20 events — showing pivot + calculated clusters)'
    : 'Live WS data — ' + heatData.eventCount + ' liquidation events (4h window)';
  const statusCol = heatData.isFallback ? 'var(--amber)' : 'var(--green)';

  // Serialize bin data for canvas drawing
  const binJson = JSON.stringify(heatData.bins.map(function(b) {
    return { p: +b.priceMid.toFixed(dec), lc: b.longCount, sc: b.shortCount, d: +b.density.toFixed(3) };
  }));

  return '<div class="card full" style="border-color:rgba(74,158,255,0.25)">'
    + '<div class="card-title" style="color:var(--blue)">Liq Heatmap'
    + '<span style="margin-left:10px;font-size:10px;color:' + statusCol + ';font-family:var(--mono)">' + statusText + '</span>'
    + '<button onclick="toggleLiqHeatmap(false);document.getElementById(\'liq-heat-toggle\').checked=false" style="float:right;font-size:9px;background:transparent;color:var(--text3);border:1px solid var(--border);border-radius:3px;padding:2px 8px;cursor:pointer;font-family:var(--mono)">Hide</button>'
    + '</div>'
    + '<div style="position:relative;width:100%">'
    + '<canvas id="liq-heatmap-canvas" height="320" style="width:100%;height:320px;display:block;border-radius:7px;background:var(--bg3)"></canvas>'
    + '</div>'
    + '<div style="display:flex;gap:16px;margin-top:8px;flex-wrap:wrap">'
    + '<span style="font-size:10px;color:var(--text3);font-family:var(--mono);display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:rgba(255,77,77,0.7);border-radius:2px;display:inline-block"></span>Long liqs (below price)</span>'
    + '<span style="font-size:10px;color:var(--text3);font-family:var(--mono);display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:rgba(0,208,132,0.7);border-radius:2px;display:inline-block"></span>Short liqs (above price)</span>'
    + '<span style="font-size:10px;color:var(--text3);font-family:var(--mono)">Darker = higher density. Current price shown as white line.</span>'
    + '</div>'
    + '</div>';
}

// Draw heatmap on canvas after DOM is ready
function drawLiqHeatmapCanvas(coin, currentPrice, klines) {
  ...
  bins.forEach(function(b, i) {
    const y = H - (i + 1) * binH; // bottom = low price, top = high price
    const isAbove = b.priceMid >= currentPrice;
    const baseCol = isAbove ? 'rgba(0,208,132,' : 'rgba(255,77,77,';
    const alpha = 0.15 + b.density * 0.75;
    ctx2d.fillStyle = baseCol + alpha.toFixed(2) + ')';
    const barW = Math.max(4, b.density * W * 0.35);
    ctx2d.fillRect(0, y, barW, binH - 1);

    // Price label for significant bins
    if (b.density > 0.45 || b.priceMid === currentPrice) {
      ctx2d.fillStyle = 'rgba(255,255,255,0.55)';
      ctx2d.font = '9px DM Mono, monospace';
      ctx2d.fillText('$' + b.p.toLocaleString(), barW + 4, y + binH * 0.65);
    }
  });

  // Current price line
  const priceY = H - ((currentPrice - minP) / (maxP - minP)) * H;
  ctx2d.strokeStyle = 'rgba(255,255,255,0.85)';
  ctx2d.lineWidth = 1.5;
  ctx2d.setLineDash([4, 3]);
  ctx2d.beginPath();
  ctx2d.moveTo(0, priceY);
  ctx2d.lineTo(W, priceY);
  ctx2d.stroke();
  ctx2d.setLineDash([]);
  ...
  ctx2d.fillText('$' + currentPrice.toLocaleString(), W - 90, priceY - 4);
}
