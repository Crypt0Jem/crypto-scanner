...
Respond ONLY in raw JSON:
{
  ...
  "positionSizeNote": "size recommendation given ${leverage}x and MTF alignment"
    ...
    if (start === -1) return res.status(500).json({ error: 'No JSON in response', raw: text.slice(0,200) });
    ...
    try {
      ...
      // Remove trailing incomplete property (find last complete property)
      partial = partial.replace(/,\s*"[^"]*"\s*:\s*[^,}\]]*$/, '');
      // Close all open braces
      partial += '}'.repeat(Math.max(0, depth));
      try {
