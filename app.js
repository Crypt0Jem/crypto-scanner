...
function renderSessionPnl() {
  ...
  if (activeW.length > 0) {
    ...
    learnHtml = '<div style="margin-top:5px;padding-top:5px;border-top:1px solid var(--border)">'
      + '<div style="font-size:9px;color:var(--purple);font-family:var(--mono);margin-bottom:3px">\u{1F9E0} Learning active</div>'
...
function showTradeLog() {
  ...
  var filtered = filtered.filter(function(t) { return t.status !== 'open'; });
...
function lockSignal(coin, tf, ta, setup, sc, price, pivots, liqZones){
  const key = coin+'_'+tf;
  // Don't re-lock if a valid signal already exists for this key
  if(swingSignals[key] && swingSignals[key].dynamic.status === 'active') return;

  const candleTime = getCurrentCandleTime(tf);

  // Determine direction at lock time from trend — don't wait for AI
  const isBullish = ta.trend === 'bullish' || ta.trend === 'mild-bullish';
  const direction = isBullish ? 'long' : 'short';

  // Use stop loss (not TA support) as the hard invalidation level — truly frozen
  const longInvalidation  = setup.lSL;   // stop loss IS the invalidation
  const shortInvalidation = setup.sSL;
  ...
  swingSignals[key] = {
    ...
    lockedSetup: {
      direction:  direction,             // ← direction locked at signal time
      bias:       direction,             // kept for compat, same as direction
      support:    ta.support,
      ...
      atr:        ta.atr,
      // Invalidation = stop loss level, frozen at lock time
      invalidationLevel:      longInvalidation,
      shortInvalidationLevel: shortInvalidation
    },
    ...
    dynamic: {
      ...
      lowScoreCandles:   0            // count by candle, not by refresh
    },
  ...
  saveSwingSignals();
  console.log('Signal locked: '+key+' @ $'+price+', score '+sc+', direction: '+direction);
}
...
function checkSignalValidity(coin, tf, currentPrice, currentScore){
  ...
  if(isNewCandle){
    ...
    // Low score check — only counts per candle, not per refresh
    if(currentScore < 5){
      sig.dynamic.lowScoreCandles = (sig.dynamic.lowScoreCandles||0) + 1;
    } else {
      sig.dynamic.lowScoreCandles = 0;
    }
    saveSwingSignals();
  }
  ...
  // RULE 2: Price hit the stop loss (frozen at lock time)
  if(direction === 'long' && ls.stopLoss && currentPrice < ls.stopLoss){
    invalidateSignal(coin, tf, 'price '+currentPrice+' hit long stop '+ls.stopLoss);
    return null;
  }
  if(direction === 'short' && ls.shortStop && currentPrice > ls.shortStop){
    invalidateSignal(coin, tf, 'price '+currentPrice+' hit short stop '+ls.shortStop);
    return null;
  }

  // RULE 3: Score weak for 3 consecutive candles (not refreshes)
  ...
  // Update status flags
  if(direction === 'long'){
    if(currentPrice <= ls.entry * 1.003 && currentPrice >= ls.entry * 0.997)
      sig.dynamic.status = 'triggered';
    if(ls.takeProfits && currentPrice >= ls.takeProfits[0])
      sig.dynamic.status = 'tp1Hit';
  } else {
    if(currentPrice >= ls.shortEntry * 0.997 && currentPrice <= ls.shortEntry * 1.003)
      sig.dynamic.status = 'triggered';
    if(ls.shortTPs && currentPrice <= ls.shortTPs[0])
      sig.dynamic.status = 'tp1Hit';
  }
  ...
  Object.keys(COINS).forEach(function(coin) {
    ...
    try {
      ...
      if (liqZ) {
        ...
        if (sweep && sweep.confirmed) {
          ...
          if (str >= 6) {
            candidates.push({ coin:coin, type:'post_sweep', label:'Post-Sweep', rawSc:rawSc, adjSc:adjSc, priority:1,
              isCounterTrend:isCounterTrend,
              detail:'Strength '+str+'/10 · '+(sweep.side==='long'?'▲ LONG':'▼ SHORT'),
              entry:sweep.entryPrice, stop:sweep.stopPrice, tp1:sweep.tp1, direction:sweep.side });
          }
        }
      }
    } catch(e) {}
    ...
    if (adjScSwing >= 6.0) {
      candidates.push({ coin:coin, type:'swing', label:'Swing', rawSc:rawSc, adjSc:adjScSwing, priority:3,
        isCounterTrend:isCounterTrend,
        detail:(ta.trend==='bullish'||ta.trend==='mild-bullish'?'▲ LONG bias':'▼ SHORT bias'),
        direction:(ta.trend==='bullish'||ta.trend==='mild-bullish'?'long':'short') });
    }
  });
...
async function renderDetail(coin,klines,mtfData){
  ...
  if(activeMode==='swing'){
    ...
    if(lockedSig){
      ...
      // Spread locked levels — direction-aware so only locked side is frozen
      ...
      // Only lock if no signal exists at all for this coin+tf
  ...
  document.getElementById('main-content').innerHTML=`
    <div class="mhdr">
      ...
      <div class="score-box">
        ${(function(){
          var adjSc = applyLearningWeight(sc, 'swing');
          var hasAdj = Math.abs(adjSc - sc) >= 0.1;
          var lw = getLearnedWeights();
          var tip = Object.keys(lw).filter(function(t){return lw[t].active;}).map(function(t){
            return lw[t].label+' '+(lw[t].weight>=1?'+':'')+((lw[t].weight-1)*100).toFixed(0)+'% ('+(lw[t].winRate||'?')+'% WR)';
          }).join(' | ') || 'Need 8+ trades per type';
          var scoreHtml = hasAdj
            ? sc+'<span style="font-size:13px;color:var(--text3)"> → </span><span style="color:'+(adjSc>=sc?'var(--green)':'var(--amber)')+'">'+adjSc+'</span>'
            : sc;
          return '<div class="score-num" style="color:'+scColor+'" title="'+tip+'">'+scoreHtml+'<span style="font-size:15px;color:var(--text3)">/10</span></div>'
            + '<div class="score-lbl">'+(hasAdj?'\u{1F9E0} Adjusted score':'Signal score')+'</div>';
        })()}
      </div>
    </div>
    ...
    <div class="content">
      ...
      <div class="card" style="border-left:3px solid var(--green)${isLocked?';border-top:1px solid rgba(245,166,35,0.4)':''}">
        <div class="card-title" style="color:var(--green);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
          <span>Long setup ${isLocked?`<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em">LOCKED ${(lockedSig&&lockedSig.lockedSetup&&lockedSig.lockedSetup.direction==='long')?'▲ LONG':''}${(lockedSig&&lockedSig.lockedSetup&&lockedSig.lockedSetup.direction==='short')?'▼ SHORT (other side)':''}</span>`:''}
          ${isLocked&&lockedSig?`<span style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px">${lockedSig.dynamic.candlesElapsed}/5 candles · SL $${lockedSig.lockedSetup.stopLoss||'—'}</span>`:''}
          </span>
      ...
      <div class="card" style="border-left:3px solid var(--red)${isLocked?';border-top:1px solid rgba(245,166,35,0.4)':''}">
        <div class="card-title" style="color:var(--red);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
          <span>Short setup ${isLocked?`<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em">LOCKED ${(lockedSig&&lockedSig.lockedSetup&&lockedSig.lockedSetup.direction==='short')?'▼ SHORT':''}${(lockedSig&&lockedSig.lockedSetup&&lockedSig.lockedSetup.direction==='long')?'▲ LONG (other side)':''}</span>`:''}
          ${isLocked&&lockedSig?`<span style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px">${lockedSig.dynamic.candlesElapsed}/5 candles · SL $${lockedSig.lockedSetup.shortStop||'—'}</span>`:''}
          </span>
        </div>
