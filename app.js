...
entryPrice:+(Math.min(curr.c, klines[klines.length-1].c)).toFixed(dec), stopPrice, stopPct,
stopNote: wickStopPct<=maxSD?'wick':'capped',
tp1:+(Math.min(curr.c,klines[klines.length-1].c)*1.0045).toFixed(dec), tp2:+(Math.min(curr.c,klines[klines.length-1].c)*1.009).toFixed(dec), tp3:+(Math.min(curr.c,klines[klines.length-1].c)*1.018).toFixed(dec),
...
entryPrice:+(Math.max(curr.c, klines[klines.length-1].c)).toFixed(dec), stopPrice:stopPriceS, stopPct:stopPctS,
stopNote: wickStopPctS<=maxSDS?'wick':'capped',
tp1:+(Math.max(curr.c,klines[klines.length-1].c)*0.9955).toFixed(dec), tp2:+(Math.max(curr.c,klines[klines.length-1].c)*0.991).toFixed(dec), tp3:+(Math.max(curr.c,klines[klines.length-1].c)*0.982).toFixed(dec),
...
const maxSDtg = setup.maxStopPct ? setup.maxStopPct/100 : 0.0096;
const lSL = +(lE * (1 - maxSDtg)).toFixed(dec);
const sE = safeNum(pe.shortEntry, setup.sE);
const sSL = +(sE * (1 + maxSDtg)).toFixed(dec);
