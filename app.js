...
async function renderDetail(coin,klines,mtfData){
  ...
  document.getElementById('main-content').innerHTML=`
    ...
    <div class="content">
      ...
      <!-- Phase 2: Sweep Opportunity secondary signal (spawns when 3-of-4 confluence) -->
      ${(()=>{
        const secKey = coin+'_'+activeTF+'_sweep';
        const sec = swingSignals[secKey];
        if(!sec) return '';
        const fn2 = n => safeFormat(n, dec);
        const isLong = sec.lockedSetup.bias !== 'bearish';
        const minsAgo = sec.evolvedReason?.spawnedAt ? Math.round((Date.now()-sec.evolvedReason.spawnedAt)/60000) : 0;
        const er = sec.evolvedReason || {};
        return `<div class="card full" style="border-color:rgba(167,139,250,0.5);background:rgba(167,139,250,0.06);border-width:2px">
          <div class="card-title" style="color:var(--purple);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
            ...
            <span style="font-size:10px;color:var(--text3);font-family:var(--mono)">${minsAgo} min ago • vol ${er.volumeMultiple||'?'}× • CVD ${er.cvdFlipped?'flipped':'neutral'} • OI ${er.oiDeltaPct>0?'+':''}${er.oiDeltaPct||0}%</span>
            <button onclick="delete swingSignals['${secKey}'];saveSwingSignals();selectCoin('${coin}')" style="font-size:9px;background:rgba(255,77,77,0.1);color:var(--red);border:1px solid var(--red-b);border-radius:3px;padding:2px 8px;cursor:pointer;font-family:var(--mono)">✕ Dismiss</button>
          </div>
          <p style="font-size:11px;color:var(--text2);font-family:var(--mono);margin:0 0 12px;line-height:1.6">Liq sweep detected at $${fn2(er.sweepPrice)} with ${er.volumeMultiple||'?'}× volume spike. Evolved from locked swing signal. Enter AFTER sweep candle closes.</p>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px">
            <div style="background:var(--bg3);border-radius:7px;padding:10px;text-align:center">
              <div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">ENTRY</div>
              <div style="font-size:15px;font-weight:700;color:var(--blue);font-family:var(--mono)">$${fn2(sec.lockedSetup.entry)}</div>
            </div>
            <div style="background:var(--bg3);border-radius:7px;padding:10px;text-align:center">
              <div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">STOP</div>
              <div style="font-size:15px;font-weight:700;color:var(--red);font-family:var(--mono)">$${fn2(sec.lockedSetup.stop)}</div>
            </div>
            <div style="background:var(--bg3);border-radius:7px;padding:10px;text-align:center">
              <div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">TP1</div>
              <div style="font-size:15px;font-weight:700;color:var(--green);font-family:var(--mono)">$${fn2(sec.lockedSetup.tp1)}</div>
            </div>
            <div style="background:var(--bg3);border-radius:7px;padding:10px;text-align:center">
              <div style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">TP2</div>
              <div style="font-size:15px;font-weight:700;color:var(--green);font-family:var(--mono)">$${fn2(sec.lockedSetup.tp2)}</div>
            </div>
          </div>
        </div>`;
      })()}

      <!-- needsReview warning banner on primary signal -->
      ${(()=>{
        const sig = swingSignals[coin+'_'+activeTF];
        if(!sig || !sig.reEval?.needsReview) return '';
        return `<div style="background:rgba(245,166,35,0.08);border:1px solid rgba(245,166,35,0.35);border-radius:8px;padding:10px 14px;margin-bottom:0;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          <span style="font-size:18px">⚠️</span>
          <div>
            <div style="font-size:12px;color:var(--amber);font-family:var(--mono);font-weight:600">Review recommended — market conditions shifted</div>
            <div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-top:2px">2+ confluence factors detected (volume spike, CVD shift, OI change). Locked levels still valid but verify before entering.</div>
          </div>
        </div>`;
      })()}
...
async function init(){
  ...
  try{
    ...
    document.getElementById('main-content').inn
