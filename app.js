  ...
  try {
    await Promise.allSettled(Object.keys(COINS).map(async function(coin) {
      try {
        ...
        if (!d || !d.price) return;
        var dec  = COINS[coin].dec;
        var kl1H = await fetchKlines(coin, '1h');
...
async function renderDetail(coin,klines,mtfData){
    ...
    <div class="content">
      ...
      <div class="card" style="border-left:3px solid var(--green)${isLocked?';border-top:1px solid rgba(245,166,35,0.4)':''}">
        <div class="card-title" style="color:var(--green);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
          <span>Long setup ${isLocked&&lockedSig?`<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em">LOCKED ${lockedSig.lockedSetup.direction==='long'?'▲ LONG':'▼ SHORT (other side)'}</span><span style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px">${lockedSig.dynamic.candlesElapsed}/5 candles · SL $${lockedSig.lockedSetup.stopLoss||'—'}</span>`:isLocked?'<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono)">LOCKED</span>':''}
          </span>
      ...
      <div class="card" style="border-left:3px solid var(--red)${isLocked?';border-top:1px solid rgba(245,166,35,0.4)':''}">
        <div class="card-title" style="color:var(--red);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px">
          <span>Short setup ${isLocked&&lockedSig?`<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em">LOCKED ${lockedSig.lockedSetup.direction==='short'?'▼ SHORT':'▲ LONG (other side)'}</span><span style="font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px">${lockedSig.dynamic.candlesElapsed}/5 candles · SL $${lockedSig.lockedSetup.shortStop||'—'}</span>`:isLocked?'<span style="font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono)">LOCKED</span>':''}
          </span>
        </div>
  ...
  renderScanBody();
  if (!IS_MOBILE) runProximityMonitor();
