...
function renderAlerts(scores){
  ...
  var alertItems = fired.map(function(e){ return '<span class="alert-item">'+e[0]+' — '+e[1]+'/10 Long setup</span>'; }).join('');
  el.innerHTML = '<div class="alerts-bar"><span class="alert-lbl">High conviction</span>' + alertItems + '</div>';
}
...
async function renderDetail(coin,klines,mtfData){
  ...
  // Pre-compute locked signal badges — no nested template literals
  var _lockedDir     = (isLocked && lockedSig && lockedSig.lockedSetup) ? lockedSig.lockedSetup.direction : null;
  var _lockedElapsed = (isLocked && lockedSig) ? (lockedSig.dynamic.candlesElapsed || 0) : 0;
  var _lockedSL      = (isLocked && lockedSig) ? (lockedSig.lockedSetup.stopLoss  || '-') : '-';
  var _lockedSSL     = (isLocked && lockedSig) ? (lockedSig.lockedSetup.shortStop || '-') : '-';
  var _lockBadgeStyle = 'font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em';
  var _lockInfoStyle  = 'font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px';
  var _lockSimple     = isLocked ? '<span style="'+_lockBadgeStyle+'">LOCKED</span>' : '';
  var _longBadge  = (isLocked && lockedSig)
    ? '<span style="'+_lockBadgeStyle+'">LOCKED ' + (_lockedDir==='long' ? '▲ LONG' : '▼ SHORT (other side)') + '</span>'
      + '<span style="'+_lockInfoStyle+'">' + _lockedElapsed + '/5 candles · SL $' + _lockedSL + '</span>'
    : _lockSimple;
  var _shortBadge = (isLocked && lockedSig)
    ? '<span style="'+_lockBadgeStyle+'">LOCKED ' + (_lockedDir==='short' ? '▼ SHORT' : '▲ LONG (other side)') + '</span>'
      + '<span style="'+_lockInfoStyle+'">' + _lockedElapsed + '/5 candles · SL $' + _lockedSSL + '</span>'
    : _lockSimple;
  var _dismissBtn = isLocked
    ? '<button onclick="invalidateSignal(\''+coin+'\',\''+activeTF+'\',\'dismissed\');selectCoin(\''+coin+'\')" style="font-size:9px;background:rgba(255,77,77,0.1);color:var(--red);border:1px solid var(--red-b);border-radius:3px;padding:2px 8px;cursor:pointer;font-family:var(--mono)">✕ Dismiss</button>'
    : '';
...
function triggerManualAI() {
  ...
  // Pre-compute AI section HTML — no nested template literals allowed
  var _patternLevels = '', _aiWatchLevel = '', _aiSuggestedAction = '';
  if (typeof rPe !== 'undefined' && typeof pe !== 'undefined') {
    _patternLevels = (pe.longEntry||pe.shortEntry)
  ...
  if (typeof ai !== 'undefined' && ai) {
    _aiWatchLevel      = ai.watchLevel     ? '<div class="watch-level">Watch level: '    + ai.watchLevel     + '</div>' : '';
    _aiSuggestedAction = ai.suggestedAction ? '<div class="action-level">Action: '       + ai.suggestedAction + '</div>' : '';
  }
