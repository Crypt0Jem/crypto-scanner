...
function renderAlerts(scores){
  ...
  var _alertItems = fired.map(function(e){ return '<span class="alert-item">'+e[0]+' — '+e[1]+'/10 Long setup</span>'; }).join('');
  el.innerHTML = '<div class="alerts-bar"><span class="alert-lbl">High conviction</span>' + _alertItems + '</div>';
}
...
async function renderDetail(coin,klines,mtfData){
  ...
  var _longBadge = isLocked&&lockedSig
    ? '<span style="'+_lockSty+'">LOCKED '+(_lDir==='long'?'▲ LONG':'▼ SHORT (other side)')+'</span>'
      +'<span style="'+_lockInf+'">'+_lN+'/5 candles · SL $'+_lSL+'</span>' : _lBase;
  var _shortBadge = isLocked&&lockedSig
    ? '<span style="'+_lockSty+'">LOCKED '+(_lDir==='short'?'▼ SHORT':'▲ LONG (other side)')+'</span>'
      +'<span style="'+_lockInf+'">'+_lN+'/5 candles · SL $'+_lSSL+'</span>' : _lBase;
