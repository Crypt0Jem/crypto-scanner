...
function showTradeLog() {
  ...
  var sigLabels = { swing:'Swing', post_sweep:'Post-Sweep', liq_scale:'Scale-In', sweep_opp:'Sweep Opp', manual:'Manual' };
  var sigColors = { swing:'var(--blue)', post_sweep:'var(--purple)', liq_scale:'var(--amber)', sweep_opp:'var(--green)', manual:'var(--text2)' };
  ...
  var formHtml = '<div id="new-trade-form" style="background:var(--bg3);border:1px solid var(--border2);border-radius:10px;padding:16px;margin-top:16px">'
    ...
    + '<option value="swing">Swing Lock</option><option value="post_sweep">Post-Sweep</option><option value="liq_scale">Liq Scale-In</option><option value="sweep_opp">Sweep Opp</option><option value="manual">Manual</option>'
    + '</select></div></div>'
  ...
  document.getElementById('main-content').innerHTML = ''
    ...
    + '<option value="all">All signals</option><option value="swing">Swing</option><option value="post_sweep">Post-Sweep</option><option value="liq_scale">Scale-In</option><option value="sweep_opp">Sweep Opp</option><option value="manual">Manual</option>'
    + '</select>'
...
async function renderDetail(coin,klines,mtfData){
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      // Draw canvas after HTML inserted — double rAF ensures layout is settled
      if(liqHeatmapOn) {
...
async function init(){
  ...
  loadTradeLog();       // restore trade history
  seedInitialTrades();  // pre-load the 8 session trades on first run
