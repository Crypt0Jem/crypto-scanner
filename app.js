...
function evaluateBestSetups() {
  ...
  Object.keys(COINS).forEach(function(coin) {
      ...
      if (liqZ) {
        ...
        if (sweep && sweep.confirmed) {
          ...
          if (str >= 6) {
            ...
            var eqsBoostPS = eqsPS ? eqsPS.boost : 0;
            var adjScPS = Math.min(10, +(adjSc + eqsBoostPS).toFixed(1));
    ...
    var swingDir = (ta.trend==='bullish'||ta.trend==='mild-bullish') ? 'long' : 'short';
    if (isCounterTrend && adjScSwing >= 7.5) adjScSwing = 7.4;
    if (adjScSwing >= 6.0) {
      var liqZSw = null;
      try { liqZSw = calcLiqZones(d.price, ta.atr||d.price*0.02, d.oi, d.funding, d.oiNotional, kl, dec); } catch(e){}
      var obSw = window._lastOB && activeCoin === coin ? window._lastOB : null;
      var eqsSw = (obSw && liqZSw) ? calcEntryQualityScore(obSw, liqZSw, d.price, ta.atr||d.price*0.02, swingDir) : null;
      var eqsBoostSw = eqsSw ? eqsSw.boost : 0;
      adjScSwing = Math.min(10, +(adjScSwing + eqsBoostSw).toFixed(1));
      if (isCounterTrend && adjScSwing >= 7.5) adjScSwing = 7.4;
      candidates.push({ coin:coin, type:'swing', label:'Swing', rawSc:rawSc, adjSc:adjScSwing, priority:3,
        isCounterTrend:isCounterTrend,
        detail:(swingDir==='long'?'▲ LONG bias':'▼ SHORT bias')+(eqsSw?' · EQ '+eqsSw.score+'/10':''),
        direction:swingDir, eqs:eqsSw });
    }
  });
...
function renderBestSetupBanner() {
  ...
  var cards = setups.map(function(s) {
    ...
    var eqsLine = s.eqs ? ' · Entry Quality '+s.eqs.score+'/10 '+s.eqs.label : '';
    ...
    return '<div style="background:'+bg+';border:1px solid '+bdr+';border-radius:8px;padding:10px 14px;margin-bottom:6px">'
      ...
      + (s.eqs ? '<div style="font-size:10px;font-family:var(--mono);margin-top:3px;color:'+eqsCol+'">'+eqsLine+' · '+s.eqs.detail+'</div>' : '')
      + '</div>';
  }).join('');
  el.style.display = 'block';
...
async function renderDetail(coin,klines,mtfData){
  ...
  const obData = IS_MOBILE ? null : await fetchOrderBook(bybitSym);
  window._lastOB = obData;
...
async function fetchOrderBook(symbol) {
  ...
  try {
    ...
    if (r && r.result) {
      _obCache[symbol] = { data: r.result, ts: now };
      return r.result;
    }
...
function calcEntryQualityScore(ob, liqZones, price, atr, direction) {
  // Returns { score: 0-10, boost: 0-2, label, detail } or null if not near zone
  if (!ob || !liqZones || !price || !atr) return null;

  // Find nearest relevant liq zone for this direction
  var zones = direction === 'long' ? liqZones.longLiqZones : liqZones.shortLiqZones;
  // Filter to 25x, 50x, 125x only (major clusters per Grok spec)
  var majorZones = zones.filter(function(z) {
    return z.leverage === '25x' || z.leverage === '50x' || z.leverage === '125x';
  });
  if (!majorZones.length) return null;

  // Find nearest zone within 1.5 ATR
  var nearZone = null;
  ...
  if (!nearZone) return null; // not within 1.5 ATR of any major zone

  // Define band: ±0.5% around the zone price
  var bandPct = 0.005;
  var bandLow  = nearZone.price * (1 - bandPct);
  var bandHigh = nearZone.price * (1 + bandPct);

  // Sum bid and ask volumes within the band
  var bids = ob.b || []; // [[price, size], ...]
  var asks = ob.a || [];

  var bidVol = 0, askVol = 0;
  bids.forEach(function(b) {
    var bp = parseFloat(b[0]);
    if (bp >= bandLow && bp <= bandHigh) bidVol += parseFloat(b[1]);
  });
  asks.forEach(function(a) {
    var ap = parseFloat(a[0]);
    if (ap >= bandLow && ap <= bandHigh) askVol += parseFloat(a[1]);
  });

  var totalVol = bidVol + askVol;
  if (totalVol < 0.0001) return { score:5, boost:0, label:'NEUTRAL', detail:'Thin book near zone', nearZone:nearZone };

  // Dominant side volume for this direction
  var dominantVol = direction === 'long' ? bidVol : askVol;
  var imbalanceScore = (dominantVol / totalVol) * 10;

  // Volume spike bonus: compare total band vol vs avg depth across all bands
  var totalBookVol = 0;
  bids.forEach(function(b) { totalBookVol += parseFloat(b[1]); });
  asks.forEach(function(a) { totalBookVol += parseFloat(a[1]); });
  var avgBandVol = totalBookVol / 50 * (bandPct * 2 * 100); // rough expected band vol
  var volBonus = 0;
  if (avgBandVol > 0) {
    var volRatio = totalVol / avgBandVol;
    if (volRatio > 2.0) volBonus =  0.8;  // strong absorption
    else if (volRatio > 1.3) volBonus = 0.4; // moderate
    else if (volRatio < 0.4) volBonus = -0.5; // thin — penalty
  }

  var rawScore = Math.min(10, Math.max(0, imbalanceScore + volBonus));
  var score    = +rawScore.toFixed(1);

  // Boost to adjScore: 0-2 points
  ...
  var detail = direction === 'long'
    ? 'Bid ' + (bidVol).toFixed(2) + ' vs Ask ' + (askVol).toFixed(2) + ' near ' + nearZone.leverage + ' zone'
    : 'Ask ' + (askVol).toFixed(2) + ' vs Bid ' + (bidVol).toFixed(2) + ' near ' + nearZone.leverage + ' zone';

  return { score:score, boost:boost, label:label, detail:detail, nearZone:nearZone,
