...
async function renderDetail(coin,klines,mtfData){
  ...
  if(activeMode==='swing'){
    ...
    if(lockedSig){
      // Use the frozen setup levels — only override prices, not structure
      const ls=lockedSig.lockedSetup;
      setup={...freshSetup,
        lE:ls.entry,   lSL:ls.stopLoss, lTP1:ls.takeProfits[0], lTP2:ls.takeProfits[1],
        sE:ls.shortEntry||freshSetup.sE, sSL:ls.shortStop||freshSetup.sSL,
        sTP1:ls.shortTPs?.[0]||freshSetup.sTP1, sTP2:ls.shortTPs?.[1]||freshSetup.sTP2,
        liqLong:ls.liqLong||freshSetup.liqLong, liqShort:ls.liqShort||freshSetup.liqShort
      };
    } else if(sc>=7){
