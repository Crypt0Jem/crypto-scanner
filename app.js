...
function renderLiqScaleInCard(coin, tf, price, ta, klines, cvdData, dec) {
  try {
    if (!klines || klines.length < 20 || !ta || !ta.atr) return '';
    const atr = ta.atr;
    const fn2 = n => safeFormat(n, dec);

    // Anchor to nearest pivot LOW below price (stable, not current price)
    const pivots = findPivotLevels(klines.slice(-60), 3);
    const anchor = (pivots.lows.filter(l => l < price).slice(-1)[0]) || price;

    // Blofin isolated margin liq levels from anchor
    const clusterTop    = +(anchor * (1 - (1/125 - 0.005 + 0.0005))).toFixed(dec);
    const clusterMid    = +(anchor * (1 - (1/50  - 0.004 + 0.0005))).toFixed(dec);
    const clusterBottom = +(anchor * (1 - (1/25  - 0.004 + 0.0005))).toFixed(dec);
    const stopLoss      = +(anchor * (1 - (1/10  - 0.003 + 0.0005)) - atr * 0.5).toFixed(dec);
...
async function renderDetail(coin,klines,mtfData){
  ...
  document.getElementById('main-content').innerHTML=`
    ...
    <div class="content">
      ...
      ${renderLiqScaleInCard(coin, activeTF, d.price, ta, klines, cvdData, dec)}
