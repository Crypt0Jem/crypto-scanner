...
function calcTradePnl(entry, exit, direction, leverage) {
  ...
  var feeDrag = 0.001 * leverage; // 0.05% taker x2 rounds to 0.1% of notional = lev*0.1% of margin
  return +((raw * leverage - feeDrag) * 100).toFixed(2);
}
...
function getTradeStats(trades) {
  ...
  // Use realizedUSDT as source of truth when present, else fall back to pnlPercent
  ...
  var sorted = trades.slice().sort(function(a,b) { return (b.realizedUSDT != null ? b.realizedUSDT : b.pnlPercent) - (a.realizedUSDT != null ? a.realizedUSDT : a.pnlPercent); });
  var byType = {};
...
function showTradeLog() {
  ...
  var formHtml = '<div id="new-trade-form" style="background:var(--bg3);border:1px solid var(--border2);border-radius:10px;padding:16px;margin-top:16px">'
    + '<div style="font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:var(--text2);font-family:var(--mono);margin-bottom:12px" id="nt-form-title">Log New Trade</div>'
    ...
    + '<div><div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">Realized USDT <span style="color:var(--text3);font-size:9px">(from BloFin — source of truth)</span></div>'
    ...
    + '<div><div style="font-size:10px;color:var(--text3);font-family:var(--mono);margin-bottom:4px">Position size USDT <span style="color:var(--text3);font-size:9px">(optional notional)</span></div>'
    ...
    + '<button onclick="cancelEditTrade()" id="nt-cancel-btn" style="display:none;padding:9px 14px;border-radius:6px;border:1px solid var(--border);background:transparent;color:var(--text2);font-family:var(--mono);font-size:12px;cursor:pointer">Cancel</button>'
  ...
  document.getElementById('main-content').innerHTML = ''
    ...
    + (stats.totalUSDT != null ? '<div style="font-size:12px;font-weight:600;color:' + (stats.totalUSDT >= 0 ? 'var(--green)' : 'var(--red)') + ';font-family:var(--mono);margin-top:2px">' + (stats.totalUSDT >= 0 ? '+' : '') + stats.totalUSDT + ' USDT</div>' : '')
...
async function renderDetail(coin,klines,mtfData){
  ...
  if(liqZones){
    ...
    if(sweep && sweep.confirmed){
      ...
      // Use localStorage (not sessionStorage) so dedup survives page refresh within same candle
      ...
      if(lastFired !== String(candleTime)){
        var swStr = calculateSweepStrength(sweep, window._lastCvdData||{}, window._lastTA||{}, mktData[coin]||{});
        if(swStr >= 6){
          localStorage.setItem(sweepKey, String(candleTime));
          sendSweepAlert(coin, sweep, ta, setup, dec);
          console.log('Sweep alert fired (strength '+swStr+'/10):', coin, sweep.side, sweep.sweepLevel);
        } else {
          console.log('Sweep alert suppressed — strength '+swStr+'/10 < 6:', coin, sweep.side);
        }
      }
    }
  }
