...
function renderAlerts(scores){
  ...
  var _alertHtml = fired.map(function(e){ return '<span class="alert-item">'+e[0]+' — '+e[1]+'/10 Long setup</span>'; }).join('');
  el.innerHTML = '<div class="alerts-bar"><span class="alert-lbl">High conviction</span>' + _alertHtml + '</div>';
}
...
async function renderDetail(coin,klines,mtfData){
  ...
  // Pre-computed vars — no nested templates or IIFEs allowed in template literals
  var _lockStyle = 'font-size:9px;background:rgba(245,166,35,0.15);color:var(--amber);border:1px solid rgba(245,166,35,0.4);border-radius:3px;padding:2px 7px;margin-left:6px;font-family:var(--mono);letter-spacing:.06em';
  var _lockInfo  = 'font-size:9px;color:var(--text3);font-family:var(--mono);margin-left:6px';
  var _lockedDir = isLocked && lockedSig && lockedSig.lockedSetup ? lockedSig.lockedSetup.direction : null;
  var _lockedN   = isLocked && lockedSig ? (lockedSig.dynamic.candlesElapsed||0) : 0;
  var _lockedSL  = isLocked && lockedSig && lockedSig.lockedSetup ? (lockedSig.lockedSetup.stopLoss||'-') : '-';
  var _lockedSSL = isLocked && lockedSig && lockedSig.lockedSetup ? (lockedSig.lockedSetup.shortStop||'-') : '-';
  var _lockBase  = isLocked ? '<span style="'+_lockStyle+'">LOCKED</span>' : '';
  var _longBadge = isLocked && lockedSig
    ? '<span style="'+_lockStyle+'">LOCKED '+(_lockedDir==='long'?'▲ LONG':'▼ SHORT (other side)')+'</span>'
      +'<span style="'+_lockInfo+'">'+ _lockedN +'/5 candles · SL $'+ _lockedSL +'</span>' : _lockBase;
  var _shortBadge = isLocked && lockedSig
    ? '<span style="'+_lockStyle+'">LOCKED '+(_lockedDir==='short'?'▼ SHORT':'▲ LONG (other side)')+'</span>'
      +'<span style="'+_lockInfo+'">'+ _lockedN +'/5 candles · SL $'+ _lockedSSL +'</span>' : _lockBase;
...
function triggerManualAI() {
  ...
  // Pre-compute pattern levels HTML
