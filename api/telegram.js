...
export default async function handler(req, res) {
  ...
  // Support comma-separated TELEGRAM_CHAT_IDS for multiple users/groups, or single TELEGRAM_CHAT_ID
  const rawIds = process.env.TELEGRAM_CHAT_IDS || process.env.TELEGRAM_CHAT_ID || '';
  const ALLOWED_IDS = rawIds.split(',').map(s => s.trim()).filter(Boolean);
  ...
  try {
    ...
    // Security: only respond to allowed chat IDs (if any are configured)
